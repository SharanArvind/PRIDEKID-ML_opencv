...
def FER_live_cam():
    ...
    # Read ONNX model
    onnx_model_path = 'Facial-Emotion-Recognition/emotion-ferplus-8.onnx'
    ...
    model_path = 'Facial-Emotion-Recognition/RFB-320/RFB-320.caffemodel'
    proto_path = 'Facial-Emotion-Recognition/RFB-320/RFB-320.prototxt'
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            for (x1, y1, x2, y2) in boxes:
               ...
               if w > 0 and h > 0 and x1 + w <= frame.shape[1] and y1 + h <= frame.shape[0]:
                face_region = gray[y1:y1 + h, x1:x1 + w]
        
               # Resize the face region for emotion recognition
               resize_frame = cv2.resize(face_region, (64, 64))
              ...
              # Calculate FPS and predict emotion
               end_time = time.time()
